...
------------------------------------------------------------
declare "tcinfo_table_nextev"
tcinfo_table_nextev = {}
declare "param_table_nextev"
param_table_nextev = {}
declare "h_sync_task_nextev"
h_sync_task_nextev = nil
declare "using_debug_nextev"
using_debug_nextev = 0

declare "change_test_model_nextev"
function change_test_model_nextev()
	tas.writestring("can1.@reopen",  "BaudRate=500000, Node=BMU, DB=C:\\Users\\abc\\Desktop\\TAS_Project\\CAN\\NextEV_BMU_CSC_CAN_Protocol_tas.xml, FrameDB=NextEV_BMU_CSC_CAN_Protocol_xnet, RevDir=0, TransceiverType=HS, Termination=1, ListenOnly=0, ReadInterval=10")
	dbc_load(4)
end

declare "set_param_nextev"
function set_param_nextev(seqid, subseqid, param_name, value)
	param_table_nextev[seqid .. "_" .. subseqid .. "_" .. param_name] = value
end

declare "get_param_nextev"
function get_param_nextev(seqid, subseqid, param_name, default_value)
	local ret = param_table_nextev[seqid .. "_" .. subseqid .. "_" .. param_name]
	if ret == nil then
...
declare "clear_param_nextev"
function clear_param_nextev()
	param_table_nextev = {}
end

declare "manual_start_bmc_sync_nextev"
function manual_start_bmc_sync_nextev()
	-- no need sync
end

declare "manual_stop_bmc_sync_nextev"
function manual_stop_bmc_sync_nextev()
	-- no need sync
end

declare "manual_init1_nextev"
function manual_init1_nextev()
	tc_nextev_1_init(true, 0)
end

declare "manual_init2_nextev"
function manual_init2_nextev()

end

declare "manual_init3_nextev"
function manual_init3_nextev()

end

declare "manual_cmc_id_assign_nextev"
function manual_cmc_id_assign_nextev(setid)

	manual_selin_freq(0, 0) -- 0V
	tas.wait(200)

	for idx = 1, 4, 1 do -- Retry with change wait time
		tas.writestring("can1.@frame:0x6C0", "021083FFFFFFFFFF")
		tas.wait(50 * idx)
		tas.writestring("can1.@frame:0x6C0", "043181F012FFFFFF")
		tas.wait(50 * idx)
		tas.writestring("can1.@frame:0x6C0", "043181F014FFFFFF")
		tas.wait(50 * idx)
		tas.writestring("can1.@frame:0x6C0", "053181F0190"..setid.."FFFF")
		tas.wait(50 * idx)
		
		local readflag = 0
		readflag = tas.read("can1.CSC_01_01")
		...
		for chkidx = 1,7,1 do
			...
			test_finish(tcid, seqno, chkidx, read_value, result)
		end
		--]]
		
		tas.wait(2000)
		
		-- if ID Assign success(readflag was changed), then break id assign logic
		if tas.read("can1.CSC_01_01") ~= readflag then
			break
		end
...
declare "manual_cmc_id_clear_nextev"
function manual_cmc_id_clear_nextev()
...
declare "manual_get_version_read_nextev"
function manual_get_version_read_nextev()
	------------------- 1. SW Version Check -------------------
	tas.writestring("can1.@frame:0x601", "0322F18900FFFFFF") -- request message
	tas.wait(100)
	
	byte_data = read_dev_msg(0x6E0) -- read SW Message
	...
	tas.writestring("can1.@frame:0x601", "30FFFFFFFFFFFFFF") -- request message
	tas.wait(100)
	...
	------------------- 2. HW Version Check -------------------
	tas.writestring("can1.@frame:0x601", "0322F193FFFFFFFF") -- request message
	tas.wait(100)
...
declare "manual_balancing_start_nextev"
function manual_balancing_start_nextev(bal_time_sec, ch_onoff_list)
	local chidx_10 = 0
	local chidx_1 = 0
	local msgidx = 1
	...
	if #ch_onoff_list > 12 then -- if input channel is over 12 channel, then send error
		...
		for chidx = 1, #ch_onoff_list, 1 do
			if ch_onoff_list[chidx] ~= nil and ch_onoff_list[chidx] == true then
				...
				chidx_1 = chidx%10
				cmc_can_write("BMU_CSC_01_02", "BMU_CSC_01_ReqBalCell_"..chidx_10..chidx_1, tonumber(bal_time_sec))
			else
				chidx_10 = math.floor(chidx/10)
				chidx_1 = chidx%10
				cmc_can_write("BMU_CSC_01_02", "BMU_CSC_01_ReqBalCell_"..chidx_10..chidx_1, 0)
			end
			tas.wait(10)
		end
	end
end
declare "manual_balancing_stop_nextev"
function manual_balancing_stop_nextev()
	local bal_state = {false,false,false,false,false,false,false,false,false,false,false,false}
	manual_balancing_start_nextev(0, bal_state)
end

declare "test_stop_nextev"
function test_stop_nextev()
	-- TODO:
	tc_nextev_25_all_power_reset(true, 100)
	cylinder_up()
end
------------------------------------------------------------
...
--------------------------------------
declare "cmc_can_init_nextev"
function cmc_can_init_nextev()
	-- No Sync Message
	tas.write("can1.onoff", 1)
end

declare "cmc_can_finish_nextev"
function cmc_can_finish_nextev()
	-- No Sync Message
...
-- 1. Initialization   
-------------------------------------------------------------------------
declare "tc_nextev_1_init"
function tc_nextev_1_init(run, seqno)
	local tcid = 1
	local unit = ""
	local p_LVCurrent = 1
	
	if run == false then
		local tcinfo = {tcid, "Initialization_nextev", "tc_nextev_1_init",
					{1, "Init", unit}
				}
		table.insert(tcinfo_table_nextev, tcinfo)
		return
	end
	...
	--tas.writestring("ao_SEL_IN.square", "3, 3, 10, 0.5")
	manual_selin_freq(12, 10) --12V 10Hz 50%
	--tas.writestring("ao_SEL_IN.square", "3.05, 2.35, 10, 0.5") --TODO	
	tas.wait(1000)
...
--  2. Current_No ID   
-------------------------------------------------------------------------
declare "tc_nextev_2_operating_current_measure"
function tc_nextev_2_operating_current_measure(run, seqno)
	local tcid = 2
	local unit = "mA"
	...
	local p_tol_min_hv = 10
	local p_tol_max_hv = 30
	...
	if run == false then
		local tcinfo = {tcid, "Current_No ID_nextev", "tc_nextev_2_operating_current_measure",
	...
	-- parameter loading
	p_tol_min_lv = get_param_nextev(seqno, 1, "Min", p_tol_min_lv)
	p_tol_max_lv = get_param_nextev(seqno, 1, "Max", p_tol_max_lv)
	p_tol_min_hv = get_param_nextev(seqno, 2, "Min", p_tol_min_hv)
	p_tol_max_hv = get_param_nextev(seqno, 2, "Max", p_tol_max_hv)
	...
	CtrlBrd(RelaySet.Power.LV_PWR_NEG, 1) -- LV PWR NEG reverted
...
--  3. CMC ID Assign
-------------------------------------------------------------------------
declare "tc_nextev_3_id_assign"
function tc_nextev_3_id_assign(run, seqno)
	local tcid = 3
	local unit_freq = "Hz"
	local unit_duty = "%"
	local unit_volt = "V"
	...
	if run == false then
		local tcinfo = {tcid, "CMC ID Assign_nextev", "tc_nextev_3_id_assign",
					{1, "CMC CAN Message", "",
	...
	-- parameter loading
	p_tol_med_bmmmsg = get_param_nextev(seqno, 1, "Med", p_tol_med_bmmmsg)
	for idx = 1, 4 do
		p_tol_min_selout[idx] = get_param_nextev(seqno, idx+1, "Med", p_tol_min_selout[idx])
		p_tol_min_selout[idx] = get_param_nextev(seqno, idx+1, "Med", p_tol_min_selout[idx])
	end
	...
	for idx = 1, 4, 1 do -- Retry with change wait time
		...
		tas.writestring("can1.@frame:0x6C0", "043181F014FFFFFF")
		tas.wait(50 * idx)
		tas.writestring("can1.@frame:0x6C0", "053181F01900FFFF")
		tas.wait(50 * idx)
...
--  4. Version check
-------------------------------------------------------------------------
declare "tc_nextev_4_version_check"
function tc_nextev_4_version_check(run, seqno)
	local tcid = 4
	local unit = ""
	...
	if run == false then
		local tcinfo = {tcid, "DEV Message Check_geelyfe3hp", "tc_nextev_4_version_check",
	...
	-- parameter loading
	p_tol_med_swversion = get_param_nextev(seqno, 1, "Med", p_tol_med_swversion)
	p_tol_med_hwversion = get_param_nextev(seqno, 2, "Med", p_tol_med_hwversion)
		
	------------------- 1. SW Version Check -------------------
	test_start(tcid, seqno, 1)
	
	--tas.writestring("can1.@frame:0x601", "0322F18900FFFFFF") -- request message
	tas.writestring("can1.@frame:0x6C0", "0322F18900000000") -- request message
	tas.wait(100)
	...
	tas.wait(100)
	--tas.writestring("can1.@frame:0x601", "30FFFFFFFFFFFFFF") -- request message
	tas.writestring("can1.@frame:0x6C0", "3000000000000000") -- request message
	tas.wait(100)
	...
	test_start(tcid, seqno, 2)
	tas.wait(100)
	tas.writestring("can1.@frame:0x6C0", "0322F19300000000") -- request message
	tas.wait(100)
...
-------------------------------------------------------------------------
declare "tc_nextev_5_lv_and_hv_current_measure"
function tc_nextev_5_lv_and_hv_current_measure(run, seqno)
	local tcid = 5
	local unit = "uA"
	local unit_op = "mA"
	...
	if run == false then
		local tcinfo = {tcid, "Current_ID Asign_nextev", "tc_nextev_5_lv_and_hv_current_measure",
	...
	-- parameter loading
	p_tol_min_lv_leak = get_param_nextev(seqno, 1, "Min", p_tol_min_lv_leak)
	p_tol_max_lv_leak = get_param_nextev(seqno, 1, "Max", p_tol_max_lv_leak)
	p_tol_min_hv_leak = get_param_nextev(seqno, 2, "Min", p_tol_min_hv_leak)
	p_tol_max_hv_leak = get_param_nextev(seqno, 2, "Max", p_tol_max_hv_leak)
	p_tol_min_lv_op = get_param_nextev(seqno, 3, "Min", p_tol_min_lv_op)
	p_tol_max_lv_op = get_param_nextev(seqno, 3, "Max", p_tol_max_lv_op)
	p_tol_min_hv_op = get_param_nextev(seqno, 4, "Min", p_tol_min_hv_op)
	p_tol_max_hv_op = get_param_nextev(seqno, 4, "Max", p_tol_max_hv_op)
...
--  6. Power Supply Test
-------------------------------------------------------------------------
declare "tc_nextev_6_power_supply_test"
function tc_nextev_6_power_supply_test(run, seqno)
	local tcid = 6
	local unit = "V"
	local strlst = {"LV(6V),HV(44.4V=3.7*12)-CELL_SUM", 
				...
				"LV(18V),HV(21.6V=1.8*12)-READ_SELOUT",
				"SELOUT Frequency", 
				"LV(18V),HV(52.8V=4.4*12)-CELL_SUM",
				"LV(18V),HV(52.8V=4.4*12)-READ_SELOUT",
				"SELOUT Frequency",
				"LV(12V),HV(44.4V=3.7*12)-READ_SELOUT",
				"SELOUT Low",
				"SELOUT Frequency",
				"SELOUT Duty"
				}
	...
	if run == false then
		local tcinfo = {tcid, "Power Supply Test_nextev", "tc_nextev_6_power_supply_test"}
	...
	-- parameter loading
	for idx = 1, 15, 1 do
		p_tol_min[idx] = get_param_nextev(seqno, idx, "Min", p_tol_min[idx])
		p_tol_max[idx] = get_param_nextev(seqno, idx, "Max", p_tol_max[idx])
	end
...
------------------------------------------------------------------------- TODO  : Spec과 맞지 않음
declare "tc_nextev_7_oddcell_no_balancing_current_consum"
function tc_nextev_7_oddcell_no_balancing_current_consum(run, seqno)
	local tcid = 7
	local unit = "A"
	local unit_cell = "mA"
	...
	if run == false then
		local subseqlst = {}
		local tcinfo = {tcid, "Cell 1,3,5,7,9,11 current consumption(No balancing)-1_nextev", "tc_nextev_7_oddcell_no_balancing_current_consum"}
		table.insert(tcinfo, 
						{1, "BMS_SUM(C)", unit,
	...
	manual_balancing_start_nextev(2, noBal)
	tas.wait(1500)
	...
	for idx = 1, 6, 1 do
		...
		read_value = (ReadCurrMean - 4 - ReadCurrLow) -- Offset Added
		read_value = read_value - read_value%0.01
...
-------------------------------------------------------------------------
declare "tc_nextev_8_oddcell_balancing_status"
function tc_nextev_8_oddcell_balancing_status(run, seqno)
	local tcid = 8
	local unit = ""
	...
	if run == false then
		local subseqlst = {}
		local tcinfo = {tcid, "Cell 1,3,5,7,9,11 balancing status_nextev", "tc_nextev_8_oddcell_balancing_status"}
	...
	-- parameter loading
	for idx=1,12,1 do
		p_tol_med[idx] = get_param_nextev(seqno, idx, "Med", p_tol_med[idx])
	end
	
	-- odd balancing
	local oddBal = {true, false, true, false, true, false, true, false, true, false, true, false}
	manual_balancing_start_nextev(1, oddBal)
	tas.wait(3000)
...
-------------------------------------------------------------------------
declare "tc_nextev_9_oddcell_balancing_current"
function tc_nextev_9_oddcell_balancing_current(run, seqno)
	local tcid = 9
	local unit = "A"
	local unit_cell = "mA"
	...
	if run == false then
		local tcinfo = {tcid, "Cell 1,3,5,7,9,11 balancing current_nextev", "tc_nextev_9_oddcell_balancing_current"}
		table.insert(tcinfo, 
						{1, "BMS_SUM(C)", unit,
							{"Min", p_tol_min[1], "Tolerance Min", "A"},
							{"Max", p_tol_max[1], "Tolerance Max", "A"}
	...
	for i = 1,3,1 do
		for j = 1, 2, 1 do
			ReadCurrMean = ReadCurrMean + cs_can_read("CellSim"..i.."SendCurrent", "CellSim"..i.."SendCurrent"..((j - 1) * 2 + 1)) -- bms odd mean sum
			ReadCurrLow = ReadCurrLow + cs_can_read("CellSim"..i.."SendCurrentLowMean", "CellSim"..i.."SendCurrentLowMean"..((j - 1) * 2 + 1)) -- bms odd low mean sum
			--tas.print("CellSim"..i.."SendCurrent, CellSim"..i.."SendCurrent"..((j - 1) * 2 + 1)) --TODO Debug
			--tas.print(cs_can_read("CellSim"..i.."SendCurrent", "CellSim"..i.."SendCurrent"..((j - 1) * 2 + 1)))
			--tas.print("CellSim"..i.."SendCurrentLowMean, CellSim"..i.."SendCurrentLowMean"..((j - 1) * 2 + 1))
			--tas.print(cs_can_read("CellSim"..i.."SendCurrentLowMean", "CellSim"..i.."SendCurrentLowMean"..((j - 1) * 2 + 1)))
		end 
	end
...
-------------------------------------------------------------------------
declare "tc_nextev_10_evencell_no_balancing_current_consum"
function tc_nextev_10_evencell_no_balancing_current_consum(run, seqno)
	local tcid = 10
	local unit = "A"
	local unit_cell = "mA"
	...
	if run == false then
		local tcinfo = {tcid, "Cell 2,4,6,8,10,12 current consumption(No balancing)_nextev", "tc_nextev_10_evencell_no_balancing_current_consum"}
		table.insert(tcinfo, 
						{1, "BMS_SUM(C)", unit,
							{"Min", p_tol_min[1], "Tolerance Min", "A"},
							{"Max", p_tol_max[1], "Tolerance Max", "A"}
	...
	local noBal = {false, false, false, false, false, false, false, false, false, false, false, false}
	manual_balancing_start_nextev(1, noBal)
	tas.wait(1500)
	...
	for i = 1,3,1 do
		for j = 1, 2, 1 do
			ReadCurrMean = ReadCurrMean + cs_can_read("CellSim"..i.."SendCurrent", "CellSim"..i.."SendCurrent"..(j * 2)) -- bms even mean sum
			ReadCurrLow = ReadCurrLow + cs_can_read("CellSim"..i.."SendCurrentLowMean", "CellSim"..i.."SendCurrentLowMean"..(j * 2)) -- bms even low mean sum
		end 
	end
...
-------------------------------------------------------------------------
declare "tc_nextev_11_evencell_balancing_status"
function tc_nextev_11_evencell_balancing_status(run, seqno)
	local tcid = 11
	local unit = ""
	local strlst = {"01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"}
	...
	if run == false then
		local tcinfo = {tcid, "Cell 2,4,6,8,10,12 balancing status_nextev", "tc_nextev_11_evencell_balancing_status"}
		for substepidx = 1, 12, 1 do
			table.insert(tcinfo, 
							{substepidx, "CSC_01_BalState_" .. strlst[substepidx], unit, 
								{"Med", p_tol_med[substepidx], "Tolerance Med", ""}
	...
	manual_balancing_start_nextev(1, evenBal)
	tas.wait(3000)
...
-------------------------------------------------------------------------
declare "tc_nextev_12_evencell_balancing_current"
function tc_nextev_12_evencell_balancing_current(run, seqno)
	local tcid = 12
	local unit = "A"
	local unit_cell = "mA"
	...
	if run == false then
		local tcinfo = {tcid, "Cell 2,4,6,8,10,12 balancing current_nextev", "tc_nextev_12_evencell_balancing_current"}
		table.insert(tcinfo, 
						{1, "BMS_SUM(C)", unit,
							{"Min", p_tol_min[1], "Tolerance Min", "A"},
							{"Max", p_tol_max[1], "Tolerance Max", "A"}
...
-------------------------------------------------------------------------
declare "tc_nextev_13_oddcell_no_balancing_current_consum_2"
function tc_nextev_13_oddcell_no_balancing_current_consum_2(run, seqno)
	local tcid = 13
	local unit = "A"
	local unit_cell = "mA"
	...
	if run == false then
		local tcinfo = {tcid, "Cell 1,3,5,7,9,11 current consumption(No balancing)-2_nextev", "tc_nextev_13_oddcell_no_balancing_current_consum_2"}
		table.insert(tcinfo, 
						{1, "BMS_SUM(C)", unit,
							{"Min", p_tol_min[1], "Tolerance Min", "A"},
							{"Max", p_tol_max[1], "Tolerance Max", "A"}
...
-------------------------------------------------------------------------
declare "tc_nextev_14_ovp_uvp"
function tc_nextev_14_ovp_uvp(run, seqno)
	local tcid = 14
	local unit = "V"
	...
	if run == false then
		local tcinfo = {tcid, "Voltage_nextev", "tc_nextev_14_ovp_uvp"}
		for substepidx = 1, 12, 1 do
			table.insert(tcinfo, 
	...
	-- parameter loading
	for idx = 1,4 do
		p_tol_min_normal[idx] = get_param_nextev(seqno, idx, "Min", p_tol_min_normal[idx])
		p_tol_max_normal[idx] = get_param_nextev(seqno, idx, "Max", p_tol_max_normal[idx])
	end
	p_tol_min_ov = get_param_nextev(seqno, 5, "Min", p_tol_min_ov)
	p_tol_max_ov = get_param_nextev(seqno, 5, "Max", p_tol_max_ov)
	p_tol_min_un = get_param_nextev(seqno, 65, "Min", p_tol_min_un)
	p_tol_max_un = get_param_nextev(seqno, 65, "Max", p_tol_max_un)

	-- Check Cell OV Normal and Over Voltage
	for idx = 1, 12, 1 do
	
		--cell simulator voltage set
		cellvolt = 3700
		...
		--selout high voltage, low voltage, frequency, duty check
		for pwmidx = 1, 4 do
...
-------------------------------------------------------------------------
declare "tc_nextev_15_voltage_accuracy_3700"
function tc_nextev_15_voltage_accuracy_3700(run, seqno)
	local tcid = 15
	local unit = "mV"
	...
	local result = false
	local cellvolt = 0
	local CellVoltageDelay = 4000
	
	if run == false then
		local tcinfo = {tcid, "Cell Voltage(3.7V)_nextev", "tc_nextev_15_voltage_accuracy_3700"}
	...
	-- parameter loading
	p_tol_min = get_param_nextev(seqno, 1, "Min", p_tol_min)
	p_tol_max = get_param_nextev(seqno, 1, "Max", p_tol_max)
	...
	for idx = 1, 12, 1 do
		...
		if idx%2 == 1 then
			read_value = tonumber(can_data) - DMM_Set(DMM.ReadVolt)*1000 -- odd cell
		else
			read_value = tonumber(can_data) + DMM_Set(DMM.ReadVolt)*1000 -- even cell
		end
...
declare "tc_nextev_16_voltage_accuracy_3400"
function tc_nextev_16_voltage_accuracy_3400(run, seqno)
	local tcid = 16
	local unit = "mV"
	...
	if run == false then
		local tcinfo = {tcid, "Cell Voltage(3.4V)_nextev", "tc_nextev_16_voltage_accuracy_3400"}
		for substepidx = 1, 24, 2 do
			table.insert(tcinfo, 
...
declare "tc_nextev_17_voltage_accuracy_4100"
function tc_nextev_17_voltage_accuracy_4100(run, seqno)
	local tcid = 17
	local unit = "mV"
	...
	if run == false then
		local tcinfo = {tcid, "Cell Voltage(4.1V)_nextev", "tc_nextev_17_voltage_accuracy_4100"}
		for substepidx = 1, 24, 2 do
			table.insert(tcinfo, 
...
------------------------------------------------------------------------- TODO : Not adapted for china spec
--[[
declare "tc_nextev_18_cvn_value_reading"
function tc_nextev_18_cvn_value_reading(run, seqno)
	local tcid = 18
	local unit = ""
	...
	if run == false then
		local tcinfo = {tcid, "CVN value reading_nextev", "tc_nextev_18_cvn_value_reading"}
...
--  19. Alarm Line Check
-------------------------------------------------------------------------
declare "tc_nextev_19_alarm_line_check"
function tc_nextev_19_alarm_line_check(run, seqno)
	local tcid = 19
	local unit = "V"
	...
	if run == false then
		local tcinfo = {tcid, "Alram Line Check_nextev", "tc_nextev_19_alarm_line_check",	
	...
	-- parameter loading
	p_tol_min_normal_selout = get_param_nextev(seqno, 1, "Min", p_tol_min_normal_selout)
	p_tol_max_normal_selout = get_param_nextev(seqno, 1, "Max", p_tol_max_normal_selout)
	p_tol_min_off_selout = get_param_nextev(seqno, 4, "Min", p_tol_min_off_selout)
	p_tol_max_off_selout = get_param_nextev(seqno, 4, "Max", p_tol_max_off_selout)
	for idx = 1,4 do
		p_tol_min_selout_pwm[idx] = get_param_nextev(seqno, idx+8, "Max", p_tol_min_selout_pwm[idx])
		p_tol_max_selout_pwm[idx] = get_param_nextev(seqno, idx+8, "Max", p_tol_max_selout_pwm[idx])
	end
	p_tol_med_selout_status = get_param_nextev(seqno, 13, "Med", p_tol_med_selout_status)
	p_tol_med_selout_remove_status = get_param_nextev(seqno, 14, "Med", p_tol_med_selout_remove_status)
	...
	cap1_enable_sig(0) -- SEL OUT Pull Up
	cap2_enable_sig(1) -- SEL OUT Pull Down
	...
	cap1_enable_sig(1) -- SEL OUT Pull Up
	cap2_enable_sig(0) -- SEL OUT Pull Down
...
-------------------------------------------------------------------------
declare "tc_nextev_20_temp_measurement"
function tc_nextev_20_temp_measurement(run, seqno)
	local tcid = 20
	local unit = "℃"
	...
	local module_num = 0
	if run == false then
		local tcinfo = {tcid, "Cell temperature_nextev", "tc_nextev_20_temp_measurement",
							{1, "Cell temperature module 1 (25℃)", unit, 
								{"Min", p_tol_min_25dgree, "Tolerance Min", unit},
								{"Max", p_tol_min_25dgree, "Tolerance Max", unit}
							},
							{2, "Cell temperature module 1 (0℃)", unit, 
								{"Min", p_tol_min_0dgree, "Tolerance Min", unit},
								{"Max", p_tol_max_0dgree, "Tolerance Max", unit}
							},
							{3, "Cell temperature module 1 (50℃)", unit, 
								{"Min", p_tol_min_50dgree, "Tolerance Min", unit},
								{"Max", p_tol_max_50dgree, "Tolerance Max", unit}
							},
							{4, "Cell temperature module 4 (25℃)", unit, 
								{"Min", p_tol_min_25dgree, "Tolerance Min", unit},
								{"Max", p_tol_min_25dgree, "Tolerance Max", unit}
							},
							{5, "Cell temperature module 4 (0℃)", unit, 
								{"Min", p_tol_min_0dgree, "Tolerance Min", unit},
								{"Max", p_tol_max_0dgree, "Tolerance Max", unit}
							},
							{6, "Cell temperature module 4 (50℃)", unit, 
								{"Min", p_tol_min_50dgree, "Tolerance Min", unit},
								{"Max", p_tol_max_50dgree, "Tolerance Max", unit}
							}
						}
	...
	-- parameter loading
	p_tol_min_25dgree = get_param_nextev(seqno, 1, "Min", p_tol_min_25dgree)
	p_tol_max_25dgree = get_param_nextev(seqno, 1, "Max", p_tol_max_25dgree)
	p_tol_min_0dgree = get_param_nextev(seqno, 2, "Min", p_tol_min_0dgree)
	p_tol_max_0dgree = get_param_nextev(seqno, 2, "Max", p_tol_max_0dgree)
	p_tol_min_50dgree = get_param_nextev(seqno, 3, "Min", p_tol_min_50dgree)
	p_tol_max_50dgree = get_param_nextev(seqno, 3, "Max", p_tol_max_50dgree)
	...
	local ch_val = 5 -- variable of resistor board id number
	...
	read_value = cmc_can_read("CSC_01_05", "CSC_01_Temp_4") -- read can temperature value
	if read_value >= p_tol_min_25dgree and read_value <= p_tol_max_25dgree then
		result = true
	else
		result = false
...
-------------------------------------------------------------------------
declare "tc_nextev_21_cell_openwire_detection"
function tc_nextev_21_cell_openwire_detection(run, seqno)
	local tcid = 21
	local unit = ""
	...
	if run == false then
		local tcinfo = {tcid, "Cell voltage wire open_nextev", "tc_nextev_21_cell_openwire_detection"}
		table.insert(tcinfo, 
	...
	-- parameter loading
	p_tol_med_normal = get_param_nextev(seqno, 1, "Med", p_tol_med_normal)
	p_tol_med_fault = get_param_nextev(seqno, 2, "Med", p_tol_med_fault)
...
-------------------------------------------------------------------------
declare "tc_nextev_22_all_power_reset"
function tc_nextev_22_all_power_reset(run, seqno)
	local tcid = 22
	local unit = ""
	...
	if run == false then
		local tcinfo = {tcid, "Power All Off & End Test_nextev", "tc_nextev_22_all_power_reset",
	...
	CtrlBrd(RelaySet.Model.nextev, 0)

	result = true

	cmc_can_finish_nextev()
	
	test_finish(tcid, seqno, 1, "", result)
	
end

declare "nextev_voltage_accuracy_test"
function nextev_voltage_accuracy_test(run, seqno)
	local tcid = 101
	local unit = "mV"
	...
	if run == false then
		local tcinfo = {tcid, "Accuracy test", "nextev_voltage_accuracy_test"}
		for substepidx = 1, 12, 1 do
			table.insert(tcinfo, 
...
declare "get_tc_list_nextev"
function get_tc_list_nextev()
	tcinfo_table_nextev = {}
	param_table_nextev = {}
	tc_nextev_1_init(false, 0)
	tc_nextev_2_operating_current_measure(false, 0)
	tc_nextev_3_id_assign(false, 0)
	tc_nextev_4_version_check(false, 0)
	tc_nextev_5_lv_and_hv_current_measure(false, 0)
	tc_nextev_6_power_supply_test(false, 0)
	tc_nextev_7_oddcell_no_balancing_current_consum(false, 0)
	tc_nextev_8_oddcell_balancing_status(false, 0)
	tc_nextev_9_oddcell_balancing_current(false, 0)
	tc_nextev_10_evencell_no_balancing_current_consum(false, 0)
	tc_nextev_11_evencell_balancing_status(false, 0)
	tc_nextev_12_evencell_balancing_current(false, 0)
	tc_nextev_13_oddcell_no_balancing_current_consum_2(false, 0)
	tc_nextev_14_ovp_uvp(false, 0)
	tc_nextev_15_voltage_accuracy_3700(false, 0)
	tc_nextev_16_voltage_accuracy_3400(false, 0)
	tc_nextev_17_voltage_accuracy_4100(false, 0)
	--tc_nextev_18_cvn_value_reading(false, 0)
	tc_nextev_19_alarm_line_check(false, 0)
	tc_nextev_20_temp_measurement(false, 0)
	tc_nextev_21_cell_openwire_detection(false, 0)
	tc_nextev_22_all_power_reset(false, 0)
	nextev_voltage_accuracy_test(false, 0)
end
get_tc_list_nextev()

declare "nextev_sequence_run"
function nextev_sequence_run()
	tc_nextev_1_init(true, 1)
	tc_nextev_2_operating_current_measure(true, 2)
	tc_nextev_3_id_assign(true, 3)
	tc_nextev_4_version_check(true, 4)
	tc_nextev_5_lv_and_hv_current_measure(true, 5)
	tc_nextev_6_power_supply_test(true, 6)
	tc_nextev_7_oddcell_no_balancing_current_consum(true, 7)
	tc_nextev_8_oddcell_balancing_status(true, 8)
	tc_nextev_9_oddcell_balancing_current(true, 9)
	tc_nextev_10_evencell_no_balancing_current_consum(true, 10)
	tc_nextev_11_evencell_balancing_status(true, 11)
	tc_nextev_12_evencell_balancing_current(true, 12)
	tc_nextev_13_oddcell_no_balancing_current_consum_2(true, 13)
	tc_nextev_14_ovp_uvp(true, 14)
	tc_nextev_15_voltage_accuracy_3700(true, 15)
	tc_nextev_16_voltage_accuracy_3400(true, 16)
	tc_nextev_17_voltage_accuracy_4100(true, 17)
	--tc_nextev_18_cvn_value_reading(true, 18)
	tc_nextev_19_alarm_line_check(true, 19)
	tc_nextev_20_temp_measurement(true, 20)
	tc_nextev_21_cell_openwire_detection(true, 21)
	tc_nextev_22_all_power_reset(true, 22)
end
